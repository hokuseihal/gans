...
def printtrain(epoch,epochs,datasize,d,loss):
    width=50
    print('\repoch:[%d/%d] loss:%4.4f'%(epoch,epochs-1,loss),'='*int(width*d/datasize),'>',sep='',end='')
def mse(x,y):
    x=x.detach().numpy()
    y=y.detach().numpy()
...
for epoch in range(epochs):
    for i in range(0,len(x_train),batchsize):
        ...
        tmp=x_train[i:i+batchsize,:]
        t=y_train[i:i+batchsize]
        loss=criterion(output,y_train[i:i+batchsize])
        lossl.append(loss.item())
        loss.backward()
        optimizer.step()
        printtrain(epoch,epochs,len(x_train),i,loss.item())
    print('')
