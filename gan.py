...
def main():
    ...
    for e in range(args.epoch):
        ...
        for i,(x, label) in enumerate(train_loader,0):
            ...
            t=torch.mean(discriminator.forward(x)).to('cpu')
            tlist.append(t)
            f=torch.mean((torch.rand(x.shape))).to('cpu')
            flist.append(f)
            tf=torch.mean(discriminator.forward(generator(torch.rand(x.shape)))).to('cpu')
            tflist.append(tf)
            #test
            print("t:",t,"f:",f,"tf",tf)
